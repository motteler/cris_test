%
% dv_from_fov5  -- dv relative to FOV 5, over a granule 
%
...
% LW data
v1 = 775; v2 = 800;  % fitting interval
robs = double(mean(squeeze(d1.rLW(:, :, 15, :)), 3));
vobs = double(d1.vLW);

% normalize over fitting interval
ix = find(v1 <= vobs & vobs <= v2);
rtmp = robs(ix, :);
vtmp = vobs(ix);
m9 = mean(rtmp);  % individual FOV means
mx = mean(m9);    % overall FOV mean
rtmp = rtmp .* (mx ./ m9);
% plot(vtmp, rtmp)
robs1 = repmat(rtmp(:,5), 1, 9);
robs2 = rtmp;  
vobs = vtmp;

% LW shift 
ppmlist = -10e-6 : 0.1e-6 : 10e-6;
...
v0 = d1.vLW;
r1 = squeeze(d1.rLW(:, :, 15, :));
b1 = real(rad2bt(v0, r1));
b1 = mean(b1, 3);
plot(v0, b1)
axis([775, 800, 270, 300])
grid on; zoom on
